{ Indy Packet Server
 ...
 3.15.22 - Indy SpaceBallz Packet Server, allowing for tournament server to be android compatible.. -q
 ...
 type
  ...
  type
    ...
    {$ENDIF}
        ...
        procedure piRecvPacket(aPacketCtx:tPacketContext);
        procedure piRecvNOP(aPacketCtx:tPacketContext);
        procedure piRecvGamer(aPacketCtx:tPacketContext);
        procedure piSendGameDef(aPacketCtx:tPacketContext);
        procedure piSendBadHash(aPacketCtx:tPacketContext);
       ...
       public
        ...
        procedure   Start;
        procedure   Stop;
        procedure   LoadGameData(aPath:String);
        procedure   SaveGameData(aPath:String);
...
procedure tPacketServer.LoadGameData(aPath: string);
var
shit:tShit;
...
procedure tPacketServer.SaveGameData(aPath: string);
var
all:tEverything;
...
procedure tPacketServer.trigGamerRecv;
begin
...
procedure tPacketServer.piRecvPacket(aPacketCtx: tPacketContext);
begin
  //process incoming packets, do a case on the incoming commands
  Log('Processing Packet Command:'+IntToStr(aPacketCtx.fHdr.Command)+' from ip:'+aPacketCtx.Context.Binding.PeerIP);

  case aPacketCtx.fHdr.Command of
  CMD_NOP:piRecvNOP(aPacketCtx);
  CMD_GMR:piRecvGamer(aPacketCtx);
  end;

end;


procedure tPacketServer.piRecvNOP(aPacketCtx: tPacketContext);
var
aBuff:tIdBytes;
begin
...
procedure tPacketServer.piRecvGamer(aPacketCtx: tPacketContext);
var
aGamerRec:tGamerRec;
aGamer:tGamer;
aIndex:integer;
              ...
              if GameData.CheckHash(aIndex,aGamer.Hash) then
                begin
                 if aGamer.BestScore>0 then
                  ...
                  piSendGameDef(aPacketCtx);
                end else piSendBadHash(aPacketCtx);
...
procedure tPacketServer.piSendGameDef(aPacketCtx: tPacketContext);
var
...
begin
   ...
   SetLength(aBuff,SizeOf(tGameDefinitionPacket));
   move(aPacket,aBuff[0],SizeOf(tGameDefinitionPacket));
   aPacketCtx.Context.Connection.IOHandler.Write(aBuff);
   setLength(aBuff,0);

end;

procedure tPacketServer.piSendBadHash(aPacketCtx: tPacketContext);
var
aPacket:TPacketHdr;
aBuff:tIdBytes;
begin
  ...
  aPacket.DataSize:=0;
  setlength(aBuff,SizeOf(tPacketHdr));
  move(aPacket,aBuff[0],SizeOf(tPacketHdr));
  aPacketCtx.Context.Connection.IOHandler.Write(aBuff);
  setlength(aBuff,0);
end;
