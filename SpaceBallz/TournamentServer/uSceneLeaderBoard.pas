...
uses dmMaterials,uGlobs,uSpaceBallzData,dmFMXPacketSrv,uDlg3dTextures;
...
//   newy:=newy+SectionHeight+aRowGap;
  ...
  tmpBitmap.Free;
      ...
      SrvCommsDm.OnRecvPacket:=UpdateBoard;
...
begin
  //
  aNewPort:=SrvCommsDm.srvSock.Port;
  if Assigned(ConfigDlg) then
    begin
     aNewPort:=ConfigDlg.Port;
     SrvCommsDM.GameData.EatGame(ConfigDlg.GameDef);
     Tron.KillConfig;
    end;

  if aNewPort<>SrvCommsDm.srvSock.Port then
    begin
      SrvCommsDM.srvSock.Close;
      SrvCommsDm.srvSock.Port:=aNewPort;
...
begin
//
     SrvCommsDm.GameData.SortGamerz;

 for I := Low(fGamerz) to High(fGamerz) do
   begin
   //
   aGamer:=SrvCommsDm.GameData.Gamer[i];
